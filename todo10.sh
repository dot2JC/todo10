...
#################### BEGIN Add Users ####################
			...
			#logger -p local3.error "${username}:password"
...
#################### BEGIN Add User to Group ####################
		...
		logger -p local3.error "User ${username} has been added to the group ${gname}"
      	fi
	done < ${2}
   echo "The number of users added is ${useradded} and the number of groups added is ${groupadded}"
   logger -p local3.error "The number of users added is ${useradded} and the number of groups added is ${groupadded}"
fi
...
then
	...
	logger -p local3.error "Do you want to delete the following users:"
   	for i in ${user_group[@]}
   	do
      		echo "${i}"
   	done
   	read -p "Yes (Y) or No (N): " delgroup </dev/tty
	logger -p local3.error ${delgroup}
   	if [ ${delgroup} = 'Y' ] || [ ${delgroup} = 'y' ]
   	then
      		echo "Are you sure you want to delete the following users:"
		logger -p local3.error "Are you sure you want to delete the following users:"
      		for i in ${user_group[@]}
      		do
         		echo "${i}"
      		done
		...
		logger -p local3.error "${delgroup}"
      		...
      		do
				...
				logger -p local3.error "User previously deleted"
				
         		else
            			echo "User ${i} has been deleted"
				logger -p local3.error "User ${i} has been deleted"
            			let ++usersdel
         		fi  
      		done
#something about how the remove groups shit aint working >:(
			...
			#echo "${val1}"
         		...
         		then
					...
					logger -p local3.error "Group previously deleted"
            			else
               				echo "Group ${i} has been deleted"
					logger -p local3.error "Group ${i} has been deleted"
               				let ++groupsdel
            			fi
         		fi
         	rm test.txt
	...
	logger -p local3.error "The number of users deleted is ${usersdel} and the number of groups deleted is ${groupsdel}"
fi
...
then
   	...
   	do
      		...
      		then
				...
				logger -p local3.error "User ${username} previously deleted"
         		else
            			echo "User ${username} has been deleted"
				logger -p local3.error "User ${username} has been deleted"
            			let ++usersdel
         		fi
         		...
         		then
					...
					logger -p local3.error "Group ${gname} has been deleted"
               				let ++groupsdel
            			fi
         		fi
         		rm test.txt
